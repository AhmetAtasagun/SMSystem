...
namespace SMSystem.Application.Features.Commands.Categories.CreateCategory
{
    public class CreateCategoryCommandHandler : IRequestHandler<CreateCategoryCommandRequest, CreateCategoryCommandResponse>
    {
        ...
        public async Task<CreateCategoryCommandResponse> Handle(CreateCategoryCommandRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                var parentCategory = await _categoryWriteRepository.Table.FindAsync(request.ParentId.Value);
                if (parentCategory == null)
                {
                    return new CreateCategoryCommandResponse().Error(_localizationService.GetLocalizedString("ParentCategoryNotFound"));
                }
            }
